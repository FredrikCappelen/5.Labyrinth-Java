...
public class generirane {
	...
	public  void crashTest(int xDirection, int yDirection) { //assign a dirrection to look in
		
		//Case trying to move left or right ( x = -1 or 1)
		if (xDirection!=0 & maze[playersCurrentRow][playersCurrentColumn  + xDirection] != 'X') {
			swapCells(playersCurrentRow, playersCurrentRow,
					playersCurrentColumn, playersCurrentColumn + xDirection);

			playersCurrentColumn += xDirection;
			playersMovesCount++;

			//Case 2 trying to move up or down ( y = -1 or 1)
		} else if (yDirection !=0 & maze[playersCurrentRow + yDirection][playersCurrentColumn] != 'X') {
			swapCells(playersCurrentRow, playersCurrentRow + yDirection,
					playersCurrentColumn, playersCurrentColumn);

			playersCurrentRow+= yDirection;
			playersMovesCount++;
		}
	...
	public  void movePlayer(char firstLetter){
		char letter = Character.toLowerCase(firstLetter);
